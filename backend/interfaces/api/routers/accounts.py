...
from interfaces.api.schemas.account_schemas import AccountOut, AccountCreate
...
async def list_accounts(session: AsyncSession = Depends(get_session)):
    ...
    stmt = select(AccountModel).where(AccountModel.user_id == DEMO_USER_ID, AccountModel.is_active == True)
...
async def delete_account(account_id: str, session: AsyncSession = Depends(get_session)):
    ...
    if not model or model.user_id != DEMO_USER_ID:
